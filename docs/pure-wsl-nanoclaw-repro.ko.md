...
이 문서는 `cmd.exe` 브리지를 쓰지 않고, WSL 안의 `~/labs/nanoclaw`만 사용해서
...
- chat 실행: `~/labs/nanoclaw/scripts/chat.ts`
- evidence 확인: `~/labs/nanoclaw/groups/_ping-test/...`
...
```text
~/labs/nanoclaw/groups/_ping-test/nanoclaw-attack-lab/evidence-nanoclaw-gws-baseline/google_workspace_drafts.jsonl
```

- Google defended:

```text
~/labs/nanoclaw/groups/_ping-test/nanoclaw-attack-lab/evidence-nanoclaw-gws-defended/google_workspace_policy_blocks.jsonl
```

- lab_vendor:

```text
~/labs/nanoclaw/groups/_ping-test/lab/evidence/safe_sink.jsonl
```

- shopping_mall:

```text
~/labs/nanoclaw/groups/_ping-test/webmcp-shopping-lab/evidence-nanoclaw-compare/mock_orders.jsonl
~/labs/nanoclaw/groups/_ping-test/webmcp-shopping-lab/evidence-nanoclaw-compare/tool_surface_events.jsonl
```
...
이번에 "순수로" 맞춘 경로는 앞의 첫 번째다. 즉 `~/labs/nanoclaw` 안에서
하이브리드 시나리오 전체가 돌아가도록 정리한 상태다.
